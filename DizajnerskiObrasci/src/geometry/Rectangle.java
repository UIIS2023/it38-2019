...
public class Rectangle extends SurfaceShape {
	...
	public boolean contains(Point p) {
		if (this.upperLeftPoint.getX() <= p.getX() && this.upperLeftPoint.getY() <= p.getY()
				&& p.getX() <= this.upperLeftPoint.getX() + width && p.getY() <= this.upperLeftPoint.getY() + height) {
			return true;
		} else {
			return false;
		}
	...
	public boolean equals(Object obj) {
		if (obj instanceof Rectangle) {
			...
			if (this.upperLeftPoint.equals(pomocni.upperLeftPoint) && this.height == pomocni.height
	...
	public String toString() {
		...
		return String.format("RECTANGLE_%1$s,%2$s_%3$sx%4$s_InnerColor_%5$s,%6$s,%7$s_OuterColor_%8$s,%9$s,%10$s",
				this.upperLeftPoint.getX(), this.upperLeftPoint.getY(), this.height,this.width,this.getInnerColor().getRed(),
